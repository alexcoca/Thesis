...
def generateIntersection(dim=2,radius=1,lower_bound=-1,upper_bound=1,num_points=5):
    ...
    def main_recursion(radius,dim,pos_lattice_coord,lb,ub,x_prev,max_dim): 
        points = []
#        if dim == max_dim:
#            radius = 1.0
#            ub = radius
...
#            x_prev= []
        ...
        for x in reversed(current_x_range):
            ...
            if dim == 2: # maybe we should think of dim == 2 as a base case?
                ...
                for partial_entry in main_recursion(radius,dim-1,pos_lattice_coord,lb,ub,x_prev,max_dim):
                    # lb = math.sqrt(radius**2/(dim-1))
                    # ub = x
    ...
    points.extend(main_recursion(radius,dim,pos_lattice_coord,lb=math.sqrt(radius/dim),ub=radius,x_prev = [],max_dim=dim))
